    ...
    <Card className="p-4 relative overflow-hidden">
      {isEmpty && !isPending && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-background/40 backdrop-blur-[2px]">
          <div className="bg-background border border-border px-6 py-3 shadow-lg flex flex-col items-center gap-2 rounded-xl">
            <span className="text-sm font-semibold text-foreground">
              No data yet
            </span>
            <p className="text-xs text-muted-foreground text-center">
              Start tracking your website to see analytics here.
            </p>
          </div>
        </div>
      )}
      <CardHeader className="flex flex-row items-stretch sm:items-start p-0!">
        {metrics?.map((metric) => (
          <div key={metric.key}>
            <button
              key={metric.key}
              ...
              className={cn(
                `relative flex flex-col flex-[0.15]! justify-center gap-1 px-4 py-4 text-left transition-colors cursor-pointer group hover:bg-primary/5`,
            ...
            >
              <span className="text-[10px] text-muted-foreground uppercase font-bold group-hover:text-primary group-data-[active=true]:text-primary transition-colors">
                {metric.label}
              </span>
