(function () {
  ...
  const ENDPOINT =
    script?.getAttribute("data-endpoint") || location.origin + "/api/collect";
  ...
  const VISITOR_KEY = "_in_vid";
  const SESSION_KEY = "_in_sid";
  let visitorId;
  let sessionId;
  ...
  const startHeartbeat = () => {
    if (!heartbeat) {
      heartbeat = setInterval(() => sendEvent("heartbeat"), 10000);
    }
  };
